...
"""
    ...
    def processPoints(self, mesh_values):
        ...
        ad2 = "{}\n{}\n{}".format(convertDistanceToDegrees(top_left), convertDistanceToDegrees(
            middle_left), convertDistanceToDegrees(bottom_left))
        ad3 = "{}\n{}\n{}".format(convertDistanceToDegrees(
            top_center), 0, convertDistanceToDegrees(bottom_center))
        ad4 = "{}\n{}\n{}".format(convertDistanceToDegrees(top_right), convertDistanceToDegrees(
            middle_right), convertDistanceToDegrees(bottom_right))

        aft1 = "Adjustment Fractional Turns"
        aft2 = "{}\n{}\n{}".format(convertDistanceToFractionalTurns(top_left), convertDistanceToFractionalTurns(
            middle_left), convertDistanceToFractionalTurns(bottom_left))
        aft3 = "{}\n{}\n{}".format(convertDistanceToFractionalTurns(
            top_center), 0, convertDistanceToFractionalTurns(bottom_center))
        aft4 = "{}\n{}\n{}".format(convertDistanceToFractionalTurns(top_right), convertDistanceToFractionalTurns(
            middle_right), convertDistanceToFractionalTurns(bottom_right))
    ...
    def mesh_level_generate(self):
        ...
        sheet_right_x = sheet_left_x + BED_SIZE_X + \
            SHEET_MARGIN_LEFT + SHEET_MARGIN_RIGHT
        sheet_front_y = -(SHEET_MARGIN_FRONT + SHEET_OFFS_Y)
        sheet_back_y = sheet_front_y + BED_SIZE_Y + \
            SHEET_MARGIN_FRONT + SHEET_MARGIN_BACK
        ...
        if len(self.mesh_level_responses) == MESH_NUM_POINTS_Y:
            ...
            img = mpimg.imread(self.get_asset_folder() +
                                '/img/mk52_steel_sheet.png')
            plt.imshow(img, extent=[sheet_left_x, sheet_right_x, sheet_front_y, sheet_back_y],
                        interpolation="lanczos", cmap=plt.cm.get_cmap(self._settings.get(["matplotlib_heatmap_theme"])))
            ...
            plt.text(0.5, 0.43, "Total Bed Variance: " + str(bed_variance) + " (mm)", fontsize=10, horizontalalignment='center',
                        verticalalignment='center', transform=ax.transAxes, bbox=dict(facecolor='#eeefff', alpha=0.5))

            # Display the adjustments below the graph
            plt.figtext(
                0.1, 0.03, adjustments[0][0], fontWeight="semibold")
            plt.figtext(0.1, -0.03, adjustments[0][1])
            plt.figtext(0.15, -0.03, adjustments[0][2])
            plt.figtext(0.20, -0.03, adjustments[0][3])

            plt.figtext(
                0.32, 0.03, adjustments[1][0], fontWeight="semibold")
            plt.figtext(0.32, -0.03, adjustments[1][1])
            plt.figtext(0.44, -0.03, adjustments[1][2])
            plt.figtext(0.56, -0.03, adjustments[1][3])

            plt.figtext(
                0.7, 0.03, adjustments[2][0], fontWeight="semibold")
            ...
            fig.savefig(self.get_asset_folder() +
                        '/img/heatmap.png', bbox_inches="tight")
